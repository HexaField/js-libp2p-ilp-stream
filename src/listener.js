...
const ilp = require('./ilp/ilp')
const EventEmitter = require('events')
const debug = require('debug')
const log = debug('libp2p:ilp:listener')
log.error = debug('libp2p:ilp:listener:error')
...
module.exports = ({ handler, upgrader }, options) => {
  ...
  const server = ilp.createServer(async socket => {
  ...
  listener.listen = ma => {
    ...
    return new Promise((resolve, reject) => {
      const options = multiaddrToILPConfig(listeningAddr)
